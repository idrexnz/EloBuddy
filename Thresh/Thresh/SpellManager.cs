...
namespace Thresh
{
    public static class SpellManager
    {
        ...
        {
            if (SpellSlot.Q.IsReady() && target != null && target.IsEnemy)
            {
                if (IsQ1)
                {
                    CastQ1(target);
        ...
        {
            if (SpellSlot.E.IsReady() && target != null && target.IsEnemy)
            {
        ...
        {
            return target != null && Util.MyHero.IsInRange(target, Smite.Range + Util.MyHero.BoundingRadius + target.BoundingRadius);
        }
    }
}