            ...
            get
            {
                ...
                if (target.IsValidTarget())
                {
                    ...
                    switch (Menu.GetSliderValue("Position"))
                    {
                        ...
                        default:
                            var turret = EntityManager.Turrets.Allies.Where(m => m.IsValidAlly() && m.Position.IsValidPosition() && m.Distance(target) - SpellManager.RKick.Range <= 750 + 200).OrderBy(m => Extensions.Distance(Util.MyHero, m, true)).FirstOrDefault();
                            if (turret != null)
                            {
