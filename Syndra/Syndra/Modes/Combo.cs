            ...
            if (Menu.GetSliderValue("R") > 0 && SpellSlot.R.IsReady())
            {
                ...
                switch (Menu.GetSliderValue("R"))
                {
                    ...
                    case 2:
                        if (damageI.IsKillable && SpellSlot.R.GetSpellDamage(target) >= target.Health)
                        {
                            if (!target.GetBestComboR().IsKillable || (Util.MyHero.HealthPercent <= target.HealthPercent && Util.MyHero.HealthPercent <= 40) || (SpellManager.Q.GetPrediction(target).CastPosition.Distance(Util.MyHero, true) >= (SpellManager.R.Range * 1.2f).Pow()))
                            {
                                SpellManager.CastR(target);
                            }
                        }
